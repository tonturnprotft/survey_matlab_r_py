...
PY
  ...
  for rep in $(seq 1 "$REPS_MORE"); do
    ...
    python run_with_time.py --out results_ode_robertson_R.csv -- \
      Rscript R/ode_robertson.R "$TEND" "$R" "$A" lsoda results_ode_robertson_R.csv "$REF"

    python run_with_time.py --out results_ode_robertson_matlab.csv -- \
      matlab -batch "addpath('$MATLAB_PATH'); ode_robertson($TEND,$R,$A,'ode15s','results_ode_robertson_matlab.csv','$REF')"
  done
done
